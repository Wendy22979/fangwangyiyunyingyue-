...
Page({
  ...
  data: {
    ...
    videoList:[],//视屏内容列表
    videoId:"",//当前视频id
    videoUpdateTime:[],//视屏播放时长获取
    isTriggered:false,//上拉状态，用于关闭下拉刷新
  ...
  async getNavList(){
   try{
    ...
    let navList = res.data.data.slice(0,14)
    this.setData({
     navList,
     navId:navList[0].id
    })
    this.getVideoList(); //请求视频
   }catch(error){
   console.log(error)
   }
  },
  ...
  handlePlay(e){
    let vid = e.target.id
    this.videoId!==vid&&this.VideoCont&&this.VideoCont.stop() //本次视频不为上次播放的视频就暂停上一个视频的播放，排除第一次视屏播放不存在的问题
     // 存储本视频id
     this.setData({
      videoId:vid
    })
    
    this.VideoCont = wx.createVideoContext(vid)  //为vid创建视屏管理实例
    let {videoUpdateTime} = this.data //如果有实例
    let videoItem = videoUpdateTime.find(item => item.vid === vid);
    if(videoItem){
      this.videoContext.seek(videoItem.currentTime);
    }
    this.VideoCont.play(); //播放视频
  ...
  handleTimeUpdate(e){
    ...
    // 存储播放时间
    let videoTimeObj = {id:e.target.id,currentTime: e.detail.currentTime}
    let flag = videoUpdateTime.findIndex((item)=> item.id === videoTimeObj.id)
    if(flag<0){
        // 添加
     videoUpdateTime.push(videoTimeObj)
    } else {
      videoUpdateTime[flag].currentTime=videoTimeObj.currentTime
    }
  ...
  handleEnded(e){
    ...
    let {videoUpdateTime} = this.data
    // 清除时长记录
    videoUpdateTime = videoUpdateTime.flite(item=>item.id !== vid)
    this.setData({
      videoUpdateTime,
    })
  },
  // 下拉刷新
  handleRefresher(){
    this.getVideoList()//更新数据
  },
  // 上拉加载，主要是分页的效果，这里数据少没分页，模拟实现
  handleToLower(){
        // 模拟数据
    let newVideoList = [
          {
